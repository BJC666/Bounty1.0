...
> Bounty 64 commits · 147 Go files · 40 packages · 全部代码亲手实现（scripts/ 下 627 个 .go 为 Eval 题库，不计入）
...
| 预览(Previewer) | ❌ | ❌ | ❌ | ❌ | ✅ |
| 输出裁剪 | ✅ 32KB | ✅ | ✅ | ✅ 动态 | ✅ 32KB |
| 内置工具数 | 22 | 25+ | 15+ | 30+ | 20+ |

**Bounty内置工具**: bash(+bash_output/bash_kill 后台轮询), read_file, write_file, edit_file, grep, glob, web_fetch, web_search(DDG), todo_write, code_index, remember, browser, task, read_only_task, fleet + 5 个 devet_* 工具
...
| 双模型Coordinator | ❌ | ❌ | ❌ | ❌ | ✅ |
| MoA多模型讨论 | ❌ | ❌ | ❌ | ✅ 60KB | ❌ |
| 多模态输入 | ✅ 文本+图片 content blocks（P4-4，三 provider 映射） | — | ✅ 视觉 | — | — |
...
| 技能安全扫描 | ✅ 8 规则拒绝加载（P4-2） | ❌ | ❌ | ✅ AST审计 | ❌ |
| 技能生命周期 | ✅ curator | ❌ | ❌ | ✅ 自动变迁 | ❌ |
| 技能触发注入 | ✅ 触发词命中自动注入技能正文（P5） | ❌ | ✅ | ✅ | ❌ |
| 插件市场 | ✅ registry+search/install/list，sha256 校验+zip-slip 防护+镜像回退（P5） | ✅ ClawHub | ✅ marketplace | ✅ SkillsHub | ❌ |
...
| 工具过滤 | ✅ 自动剥离 | ✅ | ✅ | ✅ | ✅ |
| DeVET 自动验签 | ✅ 每次 task/fleet 结果自动镜像+7 项递归验证（P4-1） | ❌ | ❌ | ❌ | ❌ |
| 验证链可视化 | ✅ Web「🛡️ 验证链」面板+SSE 实时刷新（P4-1） | ❌ | ❌ | ❌ | ❌ |
...
| CLI run | ✅ 单次执行（--json 事件流） | ✅ | ✅ | ✅ | ✅ |
| CLI doctor | ✅ 诊断 | ✅ | ❌ | ✅ | ✅ |
| 会话列表 | ✅ --list | ✅ | ✅ | ✅ | ❌ |
| 会话恢复 | ✅ --resume | ✅ | ✅ | ✅ | ❌ |
| Web仪表盘 | ✅ dashboard | ✅ React | ❌ | ✅ Vite+TS | ✅ 嵌入SPA |
| 桌面应用 | ✅ Wails实验 | ✅ 原生 | ❌ | ✅ Tauri | ✅ Wails |
| 键盘导航/输入历史 | ✅ Alt+↑↓/滚轮/翻页（P3-5） | — | — | — | — |
| 工具调用折叠面板 | ✅ 单行+展开≤8行（P3-5） | — | — | — | — |
| 彩色 diff 预览 | ✅ edit 前后对照（P3-5） | — | — | — | — |
| 权限确认弹窗 | ✅ 数字键选择/Esc 拒绝（P3-5） | — | — | — | — |
| Slash 命令 | ✅ 13 个（P3-5） | — | — | — | — |
| 会话导出 Markdown | ✅ /export（P3-5） | — | — | — | — |
| 后台长命令 | ✅ >60s 自动转后台 + bash_output 轮询 + bash_kill（P4-3） | — | — | — | — |
| run --json SDK | ✅ 逐事件 JSONL 结构化输出（P4-3） | — | — | — | — |
| CI 示例 | ✅ GitHub Action：PR 测试 + Bounty 自动审查（P4-3） | — | — | — | — |
| 粘贴图片路径 | ✅ 输入中图片路径自动识别并附为多模态消息（P4-4） | — | — | — | — |
...
| Provider | 🟢 10/11* | 🟢 10/10 | 🟡 5/10 | 🟢 10/10 | 🟡 7/10 |
| 记忆/持久化 | 🟢 8/9 | 🟡 6/9 | 🟡 2/9 | 🟢 9/9 | 🟡 5/9 |
| 插件/扩展 | 🟢 9/10 | 🟢 9/9 | 🟢 9/9 | 🟢 9/9 | 🟡 4/9 |
| 通道/消息 | 🟢 3/4 | 🟢 4/4 | 🔴 1/4 | 🟢 4/4 | 🔴 1/4 |
| Hook/生命周期 | 🟢 8/9 | 🟡 6/9 | 🟢 9/9 | 🟡 4/9 | 🟡 4/9 |
| 安全/权限 | 🟡 5/8 | 🟡 5/8 | 🟢 8/8 | 🟡 6/8 | 🟢 8/8 |
| 子代理 | 🟢 9/9* | 🟡 5/7 | 🟡 5/7 | 🟡 5/7 | 🟢 7/7 |
| 配置 | 🟢 6/6 | 🟢 6/6 | 🟡 3/6 | 🟢 6/6 | 🟢 6/6 |
| CLI/UI | 🟢 16/17* | 🟡 5/7 | 🟡 3/7 | 🟢 7/7 | 🟢 7/7 |
| 上下文管理 | 🟢 7/7 | 🟡 5/7 | 🟡 3/7 | 🟡 5/7 | 🟢 7/7 |
| 自改进 | 🟢 4/5 | 🔴 0/5 | 🔴 0/5 | 🟢 5/5 | 🔴 0/5 |
| 部署 | 🟢 7/8 | 🟢 6/8 | 🟡 3/8 | 🟢 7/8 | 🟡 4/8 |
| **总计** | **101/121*** | **86/107** | **63/107** | **95/107** | **77/107** |

🟢 ≥80% | 🟡 50-79% | 🔴 <50%

* 插件/扩展行按 10 项口径计（P5 新增「技能触发注入」，插件市场转 ✅）；其余同上。
\* CLI/UI 行：Bounty 按 17 项口径计（P3-5 新增 6 项、P4-3 新增 3 项、P4-4 新增 1 项——粘贴图片路径——均已交付，桌面应用实验计半）；Provider 行按 11 项口径计（P4-4 新增多模态输入，其余项目视觉能力未经核实标 —）；子代理行按 9 项口径计（P4-1 新增 DeVET 自动验签+可视化）；其余项目维持原口径未变，避免未经核实的评分。

## Bounty 的独特优势

| 维度 | 优势 |
|------|------|
| **代码质量** | 147个Go文件（不含 eval 题库），全手写，零依赖膨胀(Bubbletea除外)，64次独立commit |
| **架构清晰** | 40个单一职责package，依赖方向单一，接口边界明确 |
| **终端体验** | 键盘历史/工具折叠/彩色diff/权限弹窗/13个slash命令 全链路可用（P3-5） |
| **无头自动化** | run --json 事件流 + >60s 后台命令轮询（bash_output/bash_kill）+ GitHub Action PR 自动审查示例（P4-3） |
| **多模态** | 文本+图片 content blocks（OpenAI 兼容/native、Anthropic、Ollama 全映射），TUI 粘贴路径 + run --image + Web 控制台上传（P5），截图报错 Eval 3/3（P4-4） |
| **委托安全** | 唯一做到子代理结果自动镜像验签（结果承诺+授权密封+7项递归检查+精确归因），对比项中无同类能力（P4-1） |
...
| ✅ 已完成 | 插件生态 | 技能安全扫描（P4-2）、插件市场（P5）、技能触发注入（P5） |
