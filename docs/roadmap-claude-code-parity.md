...
4. **工具链补课（两处隐藏缺口）**：`report.py` 明细/失败区只遍历 A/B/C/E（D/F/G 缺失、总览缺 F/G 列）、`judge.py` 缺 `--models` 参数（weekly.py 全量路径会直接崩）——均在 P8-1/周报首次「换模型/换入口」时暴露并修复。结论与 P7 复盘一致：验收不闭环不标交付，且新工具链必须让第二条路径（不同模型、不同入口）先跑一遍。
5. **遗留项（已评估，不阻塞）**：TLSNotary/MPC-TLS v2（依赖 Rust 工具链，成本/收益与联动方案已写入 `network-proof-upgrade.md`，论文「未来工作」已引用原型实测）；多模态 G 类补测（待视觉模型接入）；周报每周固化已自动化（`weekly.py` 一键全量 → 周报 md + history.csv）。
