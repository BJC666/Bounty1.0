...
func TestPolicyAllowsNulRedirect(t *testing.T) {
	ws := t.TempDir()
	p := testPolicy(t, ws, false)
	if err := p.Check("echo hi > nul"); err != nil {
		t.Fatalf("nul redirect blocked: %v", err)
	}
	if err := p.Check("dir 2> nul"); err != nil {
		t.Fatalf("2> nul blocked: %v", err)
	}
	...
	if err := p.Check(`echo x > C:\Windows\nul.txt`); err == nil {
		t.Fatal("nul.txt outside workspace should be blocked")
	}
}
